...
const lb_rule_mgmt_1 = __nccwpck_require__(1056);
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        const ruleArn = yield lb_rule_mgmt_1.deleteLoadBalancerRule({
            tags: core_1.getInput("tags"),
            actions: core_1.getInput("actions"),
            listener: core_1.getInput("listener"),
            conditions: core_1.getInput("conditions"),
            failIfNoMatch: core_1.getInput("fail-if-no-match") == "true",
        });
        if (ruleArn) {
            core_1.info(`Deleted Rule ARN: ${ruleArn}`);
            core_1.setOutput("rule-arn", ruleArn);
        }
        return 0;
    });
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.deleteLoadBalancerRule = void 0;
const fs = __importStar(__nccwpck_require__(5747));
const util = __importStar(__nccwpck_require__(1669));
const yaml_1 = __nccwpck_require__(3552);
const elbv2_1 = __importDefault(__nccwpck_require__(4311));
function getClient() {
    return new elbv2_1.default({
...
function processConditions(conditions) {
    return processData(conditions);
}
function processActions(actions) {
    return processData(actions);
}
function processTags(tags) {
    return processData(tags);
}
...
function getRuleArnsMatchingTags(rules, tags) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return (descriptions || [])
            ...
            .reduce((accum, curr) => {
            if (curr.ruleArn && curr.match)
                accum.push(curr.ruleArn);
            return accum;
        }, matchingRuleArns);
    });
}
function compareConditions(rule, conditions) {
    const { Conditions: currentConditions } = rule;
    const conditionsToMatch = conditions.map((condition) => {
        var _a;
        const Values = (_a = condition.HostHeaderConfig) === null || _a === void 0 ? void 0 : _a.Values;
        return Object.assign({ Values }, condition);
    });
    return util.isDeepStrictEqual(currentConditions, conditionsToMatch);
}
function findMatchingRule(inputs) {
    return __awaiter(this, void 0, void 0, function* () {
        const params = { ListenerArn: inputs.listener };
        let tags = [], Rules, conditions, NextMarker, matchingRule;
        const client = getClient();
        if (inputs.conditions)
            conditions = processConditions(inputs.conditions);
        if (inputs.tags)
            tags = processTags(inputs.tags);
        const compareTags = tags.length > 1;
        do {
            ({ Rules, NextMarker } = yield client.describeRules(params).promise());
            let arnsMatchingTags = [];
            if (Rules && compareTags)
                arnsMatchingTags = yield getRuleArnsMatchingTags(Rules, tags);
            matchingRule = (Rules || []).find((rule) => {
                ...
                if (conditions)
                    matches.push(compareConditions(rule, conditions));
...
function deleteLoadBalancerRule(inputs) {
    return __awaiter(this, void 0, void 0, function* () {
        validateInputs(inputs);
        let { ruleArn, failIfNoMatch } = inputs;
        if (!ruleArn) {
            const matchingRule = yield findMatchingRule(inputs);
            if (matchingRule)
                ruleArn = matchingRule.RuleArn;
        }
        if (!ruleArn)
            return reactToNoMatchedRule(failIfNoMatch);
        yield getClient().deleteRule({ RuleArn: ruleArn }).promise();
        return ruleArn;
    });
}
exports.deleteLoadBalancerRule = deleteLoadBalancerRule;
//# sourceMappingURL=lb-rule-mgmt.js.map
...
Object.defineProperty(apiLoader.services['elbv2'], '2015-12-01', {
  get: function get() {
    var model = __nccwpck_require__(5067);
    model.paginators = __nccwpck_require__(9154)/* .pagination */ .o;
    model.waiters = __nccwpck_require__(4244)/* .waiters */ .V;
    return model;
  },
  enumerable: true,
  configurable: true
